      ...
      {activeModalData && (
        <div className="fixed inset-0 bg-slate-900/50 flex justify-center items-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg max-h-[85vh] flex flex-col p-4">
            <div className="p-2 border-b flex justify-between items-center"><h2 className="text-lg font-extrabold text-slate-800 dark:text-slate-200">{activeModalData.name}</h2><button onClick={()=>setSelectedDriverEmail(null)} className="bg-slate-100 dark:bg-slate-800 p-2 rounded-full hover:bg-slate-200"><X className="w-4 h-4"/></button></div>
              ...
              {activeModalData.jobs.length === 0 ? <p className="text-center text-sm font-bold text-slate-400">Sin traslados.</p> : activeModalData.jobs.map(j => (
                <div key={j.id} className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border text-xs relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-1 pl-2 gap-2">
                     <p className={`font-extrabold text-sm flex items-center gap-2 flex-wrap ${j.tripType === 'simple' ? 'text-purple-800' : 'text-slate-800 dark:text-slate-200'}`}>
