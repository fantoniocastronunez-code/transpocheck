...
export default function ChecklistForm({ job: rawJob, db, currentUserEmail, onCancel, onComplete, showAlert, showConfirm, allClientsList: rawClients, drivers, expenses, vehicles, uploadImageToStorage, pushSyncTask }) {
  ...
  const takeInAppPhoto = () => {
    ...
    // --- INYECCIÓN OPENCV: Enderezado y Recorte Inteligente ---
    if (inAppCamera.isScannerMode && window.cv && window.cv.Mat) {
       ...
       try {
          ...
          for (let i = 0; i < contours.size(); ++i) {
             ...
             if (area > (canvas.width * canvas.height * 0.15)) { 
                ...
                window.cv.approxPolyDP(cnt, approx, 0.02 * perimeter, true);
                
                if (approx.rows === 4 && area > maxArea && window.cv.isContourConvex(approx)) {
                   maxArea = area;
                   bestPoints = [];
                   for (let j = 0; j < 4; j++) {
          ...
          if (bestPoints) {
             ...
             let tl = bestPoints[0];
             let br = bestPoints[3];
             let remain = [bestPoints[1], bestPoints[2]];
             ...
             window.cv.warpPerspective(src, warped, M, new window.cv.Size(maxWidth, maxHeight));
          ...
          console.warn("OpenCV Falló: No se encontró un papel claro, se guardará completa.", err);
       } finally {
          if (src) src.delete();
          if (gray) gray.delete();
