...
export default function DejaVuModal({ showDejaVuModal, setShowDejaVuModal, dejaVuData, setFullScreenImage }) {
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 overflow-y-auto space-y-4">
          ...
          {dejaVuData.checklist?.observations && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 p-3 rounded-xl">
              <p className="text-[10px] font-black text-amber-700 dark:text-amber-500 uppercase mb-1">Observaciones Anteriores:</p>
                ...
                {dejaVuData.checklist.detailPins.map(pin => (
                  dejaVuData.checklist.photos?.[pin.id] && (
                    <img
                      ...
                      alt="Daño anterior"
                      onClick={() => {
                        setShowDejaVuModal(false);
                        if (setFullScreenImage) setFullScreenImage({ url: dejaVuData.checklist.photos[pin.id] });
                      }}
                    />
                  )
                ))}
          ...
          <button type="button" onClick={() => setShowDejaVuModal(false)} className="w-full py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-black rounded-xl transition-colors text-xs uppercase tracking-widest mt-2">
