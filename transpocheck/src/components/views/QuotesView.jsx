...
// Crear Traslado Automáticamente
  ...
  return (
    <div className="space-y-6 pt-24 md:pt-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {editingQuoteId && (
        <div className="bg-amber-50 dark:bg-amber-900/30 border-2 border-amber-300 dark:border-amber-700/50 p-4 rounded-2xl flex justify-between items-center shadow-sm animate-in fade-in">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 bg-amber-500 rounded-full animate-ping"></span>
            <p className="text-xs font-black text-amber-800 dark:text-amber-300">Estás modificando una cotización existente. Al guardar o aprobar, se actualizará el registro actual.</p>
          </div>
          <button onClick={() => { setEditingQuoteId(null); showAlert("Edición cancelada."); }} className="text-xs font-black text-amber-900 dark:text-amber-300 bg-amber-200 hover:bg-amber-300 px-3 py-1 rounded-xl transition-colors">Cancelar Edición</button>
        </div>
      )}
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
            <div className="flex justify-between items-center mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="text-sm font-black text-slate-800 dark:text-slate-200 flex items-center gap-2"><MapPin className="w-4 h-4 text-purple-600 dark:text-purple-400"/> Datos de Ruta y Vehículo</h3>
              <button type="button" onClick={() => setShowNewClientForm(true)} className="text-xs font-bold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400 px-3 py-1.5 rounded-xl hover:bg-purple-200 transition-colors flex items-center gap-1">
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-5">
            <h3 className="text-sm font-black text-slate-800 dark:text-slate-200 flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3"><DollarSign className="w-4 h-4 text-emerald-600 dark:text-emerald-400"/> Estructura de Costos y Peajes</h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1">
                 <label className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest ml-1">Rendimiento (KM/L)</label>
                 <div className="relative">
                   <input type="number" step="0.1" name="kmPerLiter" value={quoteData.kmPerLiter} onChange={handleInputChange} placeholder="Ej: 10.5" className="w-full border-2 border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl p-3 pl-10 text-sm font-black text-emerald-800 dark:text-emerald-300 outline-none focus:border-emerald-500"/>
              ...
              <div className="space-y-1">
                 ...
                 <div className="flex gap-2">
                   ...
                   <button 
                     ...
                     className="bg-purple-100 dark:bg-purple-900/40 hover:bg-purple-200 text-purple-700 dark:text-purple-400 px-3 rounded-xl text-[10px] font-black shadow-sm shrink-0 transition-colors disabled:opacity-50 flex items-center gap-1 uppercase" 
              ...
              <div className="space-y-1">
                 ...
                 <div className="relative">
                   <input type="number" name="driverFee" value={quoteData.driverFee} onChange={handleInputChange} placeholder="Honorarios..." className="w-full border-2 border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-900/30 rounded-xl p-3 pl-10 text-sm font-bold text-blue-800 dark:text-blue-300 outline-none focus:border-blue-500"/>
                   <User className="w-4 h-4 text-blue-500 absolute left-3.5 top-3.5" />
                 </div>
              </div>

              <div className="space-y-1">
                 <label className="text-[10px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest ml-1">Margen Empresa (%)</label>
                 <div className="relative">
                   <input 
                     type="number" 
                     step="0.01"
                     ...
                     className="w-full border-2 border-purple-200 dark:border-purple-800/50 bg-purple-50 dark:bg-purple-900/30 rounded-xl p-3 text-sm font-black text-purple-800 dark:text-purple-300 outline-none focus:border-purple-500"
                   />
                 </div>
              </div>

              <div className="space-y-1">
                 <label className="text-[10px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest ml-1">Forzar Neto ($)</label>
                 <div className="relative">
                   <input 
                     type="text" 
                     ...
                     className="w-full border-2 border-purple-200 dark:border-purple-800/50 bg-purple-50 dark:bg-purple-900/30 rounded-xl p-3 pl-10 text-sm font-black text-purple-800 dark:text-purple-300 outline-none focus:border-purple-500"
                   />
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-4">
              <div className="flex justify-between items-center mb-3">
                ...
                <button type="button" onClick={() => { setEditingTollId(null); setShowNewTollModal(true); }} className="text-[11px] font-bold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400 px-3 py-1.5 rounded-xl hover:bg-purple-200 transition-colors flex items-center gap-1">
              ...
              {tollsList.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-1 scrollbar-none border border-slate-100 dark:border-slate-800 rounded-2xl bg-slate-50 dark:bg-slate-900/50">
                  {tollsList.map(toll => {
                    ...
                    return (
                      <div key={toll.id} className={`flex items-center justify-between p-2.5 rounded-xl border transition-all ${isChecked ? 'bg-purple-50 dark:bg-purple-900/30 border-purple-300 dark:border-purple-700/50 shadow-sm' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700'}`}>
                        ...
                        <div className="flex items-center gap-2 shrink-0 ml-2">
                          <span className="text-xs font-black text-purple-700 dark:text-purple-400">{formatMoney(price)}</span>
                          <button type="button" onClick={() => handleEditToll(toll)} className="text-[10px] font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400 px-2 py-1 rounded-lg transition-colors">Modificar</button>
                        </div>
                      </div>
                    );
                  })}
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 mt-8">
        <h3 className="text-base font-black text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2"><Receipt className="w-5 h-5 text-purple-600 dark:text-purple-400"/> Historial de Cotizaciones ({savedQuotes.length})</h3>
                    ...
                    <tr key={q.id} className="hover:bg-slate-50 dark:bg-slate-900/50">
                      ...
                      <td className="p-3">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                          q.status === 'aceptada' ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400' :
                          q.status === 'enviada' ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400' : 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400'
                        }`}>
                          {q.status || 'pendiente'}
                        </span>
                      </td>
                      <td className="p-3 text-right relative">
                        <button onClick={() => setActiveActionMenu(activeActionMenu === q.id ? null : q.id)} className="p-2 hover:bg-slate-200 dark:bg-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl transition-colors shadow-sm">
                        ...
                        {activeActionMenu === q.id && (
                          <div className={`absolute right-12 ${idx > 0 && idx === savedQuotes.length - 1 ? 'bottom-10' : 'top-10'} bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl rounded-xl w-48 py-2 z-[99] animate-in fade-in zoom-in-95`}>
                            ...
                            <button onClick={() => { handleDeleteQuote(q.id); setActiveActionMenu(null); }} className="w-full text-left px-4 py-2.5 text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:bg-red-900/30 transition-colors">Eliminar</button>
                          </div>
                        )}
                      </td>
                    </tr>
      ...
      {showNewClientForm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
          <form onSubmit={handleCreateNewClient} className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl relative animate-in zoom-in-95">
            <button type="button" onClick={() => setShowNewClientForm(false)} className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:bg-slate-700 transition-colors"><X className="w-4 h-4 text-slate-700 dark:text-slate-300"/></button>
      ...
      {showSendModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl relative animate-in zoom-in-95">
            <button type="button" onClick={() => setShowSendModal(false)} className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:bg-slate-700 transition-colors"><X className="w-4 h-4 text-slate-700 dark:text-slate-300"/></button>
            <h3 className="text-lg font-black text-slate-800 dark:text-slate-200 mb-2">Enviar Cotización</h3>
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-5">Elige cómo enviar esta cotización. Al enviarla o descargarla, se guardará automáticamente como <span className="text-blue-600 dark:text-blue-400">ENVIADA</span>.</p>
            ...
            <div className="space-y-3">
              ...
              <button 
                ...
                className="w-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-black py-3.5 rounded-2xl transition-colors shadow-sm flex items-center justify-center gap-2 text-sm"
              >
              ...
              <button 
                ...
                className="w-full bg-purple-100 dark:bg-purple-900/40 hover:bg-purple-200 border border-purple-200 dark:border-purple-800/50 text-purple-700 dark:text-purple-400 font-black py-3.5 rounded-2xl transition-colors shadow-sm flex items-center justify-center gap-2 text-sm"
              >
                <Printer className="w-5 h-5 text-purple-600 dark:text-purple-400"/> Descargar PDF
              </button>
            </div>
          </div>
        </div>
      ...
      {showNewTollModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
          <form onSubmit={handleSaveNewToll} className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-lg shadow-2xl relative animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <button type="button" onClick={() => setShowNewTollModal(false)} className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:bg-slate-700 transition-colors"><X className="w-4 h-4 text-slate-700 dark:text-slate-300"/></button>
                  ...
                  {Object.keys(newTollData.prices).map(vType => (
                    <div key={vType} className="bg-slate-50 dark:bg-slate-900/50 p-2 rounded-xl border border-slate-200 dark:border-slate-700">
                      ...
                      <input 
                        ...
                        placeholder="$ Costo" 
                        className="w-full mt-1 border border-slate-300 dark:border-slate-600 rounded-lg p-1.5 text-xs font-black text-purple-700 dark:text-purple-400 outline-none focus:border-purple-500 bg-white dark:bg-slate-900"
                      />
                    </div>
                  ))}
                </div>
      ...
      {showAcceptModal && acceptQuoteData && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
          <form onSubmit={handleConfirmAcceptQuote} className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl relative animate-in zoom-in-95">
            <button type="button" onClick={() => setShowAcceptModal(false)} className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:bg-slate-700 transition-colors"><X className="w-4 h-4 text-slate-700 dark:text-slate-300"/></button>
