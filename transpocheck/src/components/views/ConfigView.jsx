          ...
          <form key={editingToll ? editingToll.id : 'new-toll'} onSubmit={async (e) => {
             ...
             const data = {
                ...
                prices: {
                   ...
                   'Camión': Number(fd.get('priceCamion')) || 0
                }
             };
            ...
            <div className="grid grid-cols-2 gap-3 bg-emerald-50 p-3 rounded-2xl border border-emerald-100">
               <div className="col-span-2 text-[10px] font-black text-emerald-600 uppercase tracking-widest border-b border-emerald-200/50 pb-1 mb-1">Valores por Vehículo</div>
               <div className="space-y-1">
               ...
               <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-600 ml-1">Camión ($)</label>
                  <input name="priceCamion" type="number" defaultValue={editingToll?.prices?.['Camión']} placeholder="0" required className="w-full border border-emerald-200 p-2 rounded-lg text-sm outline-none focus:border-emerald-500 font-bold bg-white"/>
               </div>
            </div>
                ...
                <div key={t.id} className="flex justify-between items-center p-3 bg-slate-50 border border-slate-100 rounded-xl hover:border-emerald-200 transition-all group">
                  <div className="flex-1 min-w-0 pr-2">
                    ...
                    <div className="flex gap-1.5 mt-1.5 flex-wrap">
                       <span className="text-[9px] font-bold bg-white border border-slate-200 text-slate-600 px-1.5 py-0.5 rounded">🚙 ${t.prices?.['Auto / SUV']}</span>
                       <span className="text-[9px] font-bold bg-white border border-slate-200 text-slate-600 px-1.5 py-0.5 rounded">🛻 ${t.prices?.['Camioneta']}</span>
                       <span className="text-[9px] font-bold bg-white border border-slate-200 text-slate-600 px-1.5 py-0.5 rounded">🚚 ${t.prices?.['Camión']}</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-1.5 shrink-0 ml-2">
