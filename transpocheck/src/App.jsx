...
const SignaturePad = ({ onSave, onClear, initialData }) => {
  ...
  useEffect(() => {
    ...
    ctx.lineWidth = 4; // Grosor fijo anti-temblores
    ctx.lineCap = 'round'; 
    ctx.lineJoin = 'round';
  ...
  return (
    <div className={isFullscreen 
      ...
      {/* HEADER: Solo visible en pantalla completa */}
      {isFullscreen && (
      ...
      {/* CONTENEDOR CENTRAL ADAPTABLE (Nunca empujará los botones fuera de la pantalla) */}
      <div className={`relative w-full mx-auto flex items-center justify-center shrink-0 ${isFullscreen ? 'flex-1 min-h-0' : 'aspect-[2/1]'}`}>
         ...
         {/* El canvas se auto-escala manteniendo su proporción sin desbordar su caja padre */}
      ...
      {/* FOOTER DE BOTONES (Fijados al fondo) */}
    ...
    return (
      <div key={j.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-between relative pl-5 overflow-hidden hover:shadow-xl hover:-translate-y-1 active:scale-[0.98] transition-all duration-300 cursor-default">
        ...
        <div className="flex gap-1.5 mt-auto">
          ...
          <button onClick={() => handleShareWhatsAppPDF(j)} disabled={processingId === `${j.id}-wapp`} className="flex-1 py-1.5 flex justify-center bg-green-50 text-green-700 hover:bg-green-100 rounded-lg transition-colors disabled:opacity-50" title="Compartir WhatsApp"><Share2 className="w-3.5 h-3.5"/></button>
