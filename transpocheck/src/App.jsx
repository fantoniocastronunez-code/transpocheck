...
export default function App() {
  ...
  const [jobsSubTab, setJobsSubTab] = useState('list'); // list, create
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-24 font-sans">
      ...
      <header className="bg-blue-600 text-white p-4 shadow-lg flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-3"><div className="bg-white/20 p-2 rounded-xl backdrop-blur-sm"><Car className="w-6 h-6 text-white" /></div><h1 className="font-extrabold text-2xl tracking-tight hidden sm:block">LogisticAPP</h1></div>
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {isRealAdmin && (
            <button onClick={() => { setActiveRole(activeRole === 'admin' ? 'driver' : 'admin'); setMainTab('jobs'); setJobsSubTab('list'); }} className="flex items-center gap-1.5 bg-white/20 hover:bg-white/30 px-3 py-2 rounded-xl text-sm font-bold transition-all border border-white/10 backdrop-blur-sm">
      ...
      {editingJob && <EditJobModal job={editingJob} drivers={drivers} db={db} onClose={() => setEditingJob(null)} showAlert={showAlert} />}
...
// ==========================================
// COMPONENTES DE VISTAS (RANKING, EXPENSES, JOBS, CHECKLIST)
// ==========================================
const getRouteStr = (j) => {
...
function ExpensesView({ role, drivers, jobs, expenses, db, currentUserEmail, showAlert, showConfirm }) {
  ...
  // HELPER PARA FECHAS SEGURAS EN iOS (SAFARI)
  ...
  if (isAdminView) {
    return (
      <main className="max-w-5xl mx-auto p-4 pt-6 pb-24">
        {viewingReceipt && <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4"><div className="bg-white p-4 rounded-3xl w-full max-w-sm relative"><button onClick={()=>setViewingReceipt(null)} className="absolute top-2 right-2 bg-slate-200 p-1 rounded-full"><X className="w-4 h-4 text-slate-700"/></button><h3 className="font-extrabold text-slate-800 mb-4 ml-2">Comprobante</h3><img src={viewingReceipt} className="w-full h-auto rounded-xl shadow-sm"/></div></div>}
            ...
            <div className="overflow-y-auto space-y-3 flex-1 pr-1" style={{ maxHeight: '60vh' }}>
              {expenses.filter(e => selectedDriverId ? e.driverId === selectedDriverId : true).map(exp => (
                <div key={exp.id} className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex gap-3 items-start text-xs font-bold w-full overflow-hidden">
                  <div className="mt-1">{getTransactionIcon(exp.type)}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-slate-800 break-words">{exp.detail}</p>
                    <p className="text-[10px] text-slate-400 truncate">{!selectedDriverId && <span className="text-blue-600">{exp.driverName} • </span>}{formatSafeDate(exp.createdAt)}</p>
                    {exp.receiptImage && <button onClick={() => setViewingReceipt(exp.receiptImage)} className="mt-1.5 flex items-center gap-1.5 text-[10px] text-blue-600 bg-blue-100/50 px-2 py-1 rounded-md transition-colors w-fit"><Camera className="w-3.5 h-3.5"/> Ver comprobante</button>}
                  </div>
  ...
  return (
    <main className="max-w-md mx-auto p-4 pt-6 space-y-6 pb-24">
      ...
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-3xl shadow-md text-center text-white relative overflow-hidden">
        ...
        <p className="font-bold uppercase tracking-wider text-xs mb-1">Fondo Asignado Actual</p>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        <form onSubmit={e=>addExp(e,'expense',Number(e.target.amount.value), e.target.detail.value, myDriver.id, myDriver.name, myDriver.email)} className="space-y-4">
          <input type="text" name="detail" placeholder="¿En qué gastaste? (Ej. Peaje)" required className="w-full border-2 border-slate-200 p-4 rounded-xl outline-none focus:border-blue-500 font-bold text-sm text-slate-700" />
          <input type="number" name="amount" placeholder="Monto ($)" required className="w-full border-2 border-slate-200 p-4 rounded-xl outline-none focus:border-blue-500 font-bold text-sm text-slate-700" />
          <button type="submit" disabled={myBalance <= 0 || hasPendingReturn} className={`w-full py-4 rounded-2xl font-extrabold text-sm transition-all ${myBalance > 0 && !hasPendingReturn ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-200' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>Guardar Gasto</button>
        </form>
      </div>
          ...
          {expenses.filter(e => e.driverId === myDriver.id).map(exp => (
            <div key={exp.id} className="flex items-start gap-4 p-3 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                {exp.receiptImage && <button onClick={() => setViewingReceipt(exp.receiptImage)} className="mt-1.5 flex items-center gap-1.5 text-[10px] text-blue-600 bg-blue-100/50 px-2 py-1 rounded-md transition-colors w-fit"><Camera className="w-3.5 h-3.5"/> Ver foto</button>}
              </div>
